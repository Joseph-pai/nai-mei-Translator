  ...
  persist(
    (set, get) => ({
      ...
      validateApiKey: async (provider, apiKey) => {
        ...
        try {
          // 調用真正的 AI 驗證邏輯
          const { AIAdapter } = await import('./ai-service')
          const isValid = await AIAdapter.validateKey(provider, apiKey)
          ...
          if (isValid) {
            ...
            set({ validationError: 'API Key 驗證失敗，請檢查 Key 是否正確或網絡是否通暢' })
            return false
          }
        } catch (error) {
          set({ validationError: '驗證過程中發生錯誤，請稍後重試' })
          return false
        } finally {
